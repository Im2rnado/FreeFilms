import { MediaShort } from '../types/Media';

function has(id: string, type: string) {
...
function on(id: string, type: string, callback: () => void) {
    window.addEventListener(`wishlist-${type}-${id}`, callback);
}

function off(id: string, type: string, callback: () => void) {
